...
const PackageCard = ({ icon, title, description, price, services, addOns }) => (
  <div className="w-full px-4 md:w-1/3 lg:w-1/3">
    <div className="mb-9 rounded-[20px] bg-white p-10 shadow-md hover:shadow-lg md:px-7 xl:px-10">
      ...
      <h5 className="mt-6 text-xl font-semibold">Add Ons:</h5>
      <ul className="mt-2 text-gray-700 light:text-gray-300 text-lg">
        {addOns.map((addOn, index) => (
          <li key={index} className="mb-2">• {addOn}</li>
        ))}
      </ul>
    </div>
  </div>
